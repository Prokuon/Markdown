...
* 任何一个微处理器都要与一定数量的部件和外围设备连接，但如果将各部件和每一种外围设备都分别用一组线路与CPU直接连接，那么连线将会错综复杂，甚至难以实现。为了简化硬件电路设计、简化系统结构，常用一组线路，配置以适当的接口电路，与各部件和外围设备连接，这组共用的连接线路被称为总
* **微机中总线一般有==内部总线==、==系统总线==和==外部总线==。计算机常用总线有**PCIe、PCI、USB、SATA和DMI

#### 1.4 linux下编译程序

**Q**: 我们用的是Ubuntu 16.04版本，Ubuntu是一个以桌面应用为主的Linux操作系统，其内核是Linux。 从前人们认为Linux难以安装、难以使用，在Ubuntu出现后这些都成为了历史。Ubuntu也拥有庞大的社区力量，用户可以方便地从社区获得帮助。Ubuntu常常作为linux爱好者来学习linux应用编程和linux驱动开发的平台。
...
上面的-o foo选项是告诉编译器可执行程序的名字，就是后面生成的foo文件，如果你忘记提这个，那么编译器会把程序放在一个名为: a.out的文件里。-Wall表示输出一些简单的错误以及一些可能存在问题的警告。

**A**: 当我们有多个文件时，怎么链接到一起最后形成一个执行文件呢? 

**Q**: 这个问题问的很好。我们给出实例，我们创建两个c文件，一个foo.c和max.c，foo.c里main函数调用max.c里的max比较大小函数: 
...
```c
$ gcc -Wall foo.c max.c -o foo
```

该命令自动将两个文件编译、链接并生成执行文件，运行./foo
...
这里的-c就表示只执行到汇编这一步而不链接，生成目标文件foo.o；max.c文件同理生成max.o，最后我们将两个目标文件链接起来生成可执行文件。A，关于程序编译的四个步骤，你可没忘吧? 

```c
$ gcc -c foo.c -o foo.o
$ gcc -c max.c -o max.o
$ gcc max.o foo.o -o foo
$ ./foo  
Hello, Douya!  
the max value is 6:  
```
...
//在 tmp目录下创建路径为test/t1/foo的目录  
mkdir -p /tmp/test/t1/foo  
```
...
5. User模式和Supervisor模式有什么区别⭐⭐⭐
   ...
   **用户模式user**是用户程序的工作模式，它运行在操作系统的用户态，它没有权限去操作其它硬件资源，只能执行处理自己的数据，也不能切换到其它模式下，要想访问硬件资源或切换到其它模式==只能通过**软中断**或产生**异常**==。
...
7. 说说DMA⭐⭐⭐⭐
   ...
   1. 概念
      ...
      为I/O使用一种特殊的直接存储器访问(Direct Memory Access，**DMA**)芯片，它可==以直接控制外围设备的数据流==，而无需持续的CPU干预。这样效率就很高了，但对应成本就相对高些，因为DMA是由专门的硬件(DMA)控制。
...
9. 文件权限怎么修改⭐⭐⭐
   ...
   **答案解析**
   ...
   # chmod 770 foo.c //即修改foo.c文件的权限为770
   ```
...
**Q**: 进程有五种状态: ==创建、就绪、执行、阻塞、终止==。一个进程创建后，被放入队列处于就绪状态，等待操作系统调度执行，执行过程中可能切换到阻塞状态(并发)，任务完成后，进程销毁终止。如图: 
...
1. 我们先讲==由操作系统创建的进程==。在系统启动时，操作系统会创建一些进程，它们承担着管理和分配资源的任务，这些进程维持这系统的稳定运行，被称为**系统进程**。

2. 另一种方式就是==由**父进程创建**==。系统允许一个进程创建新进程(即子进程)，子进程又可以创建新的子进程，形成树结构。子进程创建成功后，子进程将存在于系统之中，并且独立于父进程。子进程可以接受系统调度，可以分配资源。那么创建一个子进程，常用**fork()函数**，其原型如下: 
...
**Q**: 接下来我们给出用例: 这个例子让父子进程分别获取PID。A，快点记。
...
`getpid()`用于获得当前进程的pid，而ge`tppid()`则是获取父进程的pid。
...
**Q**: **系统进程**，其中有一种特殊的进程，那就是**==守护进程==**，面试也喜欢问，我们就来介绍一下。
...
2. **调用`setsid()`创建一个新会话**。控制终端、登录会话和进程组通常是从父进程继承下来的，守护进程要摆脱它们，不受它们的影响，方法是调用setsid() 使进程成为一个会话组长。`setsid()` 调用成功后，进程成为新的会话组长和进程组长，并与原来的登录会话、进程组和控制终端脱离。
...
3. **将当前目录更改为根目录**。使用`fork()`创建的子进程也继承了父进程的当前工作目录。由于在进程运行过程中，当前目录所在的文件系统不能卸载，因此，把当前工作目录换成其他的路径，如根目录。
...
我们给出一个实例: 实现一个简单的守护进程(每隔2s在/tmp/foo.c中写入一句话)
...
int main(){  
    ...
    while (1){  
        int fd;  
        if ((fd=open ("/tmp/foo.c", O_CREAT|O_WRONLY|O_APPEND, 0600)) < 0){ 
            perror("open");  
            exit (1) ;  
        }  
        write(fd, "processing...\n", strlen("processing...\n")+1);  
...
实际上发现我们的程序依然运行着，只不过在后台运行。我们查看"/tmp/foo.c"下的日志，可以看到，进程每隔2s写入"processing...\n"。
...
int main(){    
    ...
    if (pid < 0){    
        ...
        //子进程向父进程写数据，关闭管道的读端   
        close(fd[INPUT]);  
        write(fd[OUTPUT], "hello foo!", strlen("hello foo!"));  
        exit(0);  
    }  
    else{//执行父进程  
...
13 bytes of data from child process: hello foo!
```
...
read:hello!foo!
$ ./process2
Please input something:hello!foo!
write:hello!foo!
```
...
**Q**: (笑容邪魅，语速加快)这个实例创建了一个线程，调用回调函数打印“hello foo！”和传入的参数，同时捕获线程id。pthread.c实例如下: 
...
hello foo, i = 1, tid = 140669279119104
retval = 1
```
...
然后运行客户端，输入./client 127.0.0.1，说明经过了TCP三次握手后，已经成功建立了连接，客户端等待我们发送消息，我们输入：“hello，foo！”
...
1. 程序是怎么编译成bin文件的
   ...
   1. 先用gcc把foo.c编译成foo.o：`gcc -c foo.c -o foo.o`
   2. 将foo.o转为bin文件：`objcopy -O binary foo.o foo.bin`
